...
public class JREUtils {
    ...
    public static LibraryPlugin getMobileGluesPlugin(Context ctx) {
        LibraryPlugin mobileGlues = LibraryPlugin.discoverPlugin(ctx, LibraryPlugin.ID_MOBILEGLUES_PLUGIN);
        if(mobileGlues == null || !mobileGlues.checkLibraries("libmobileglues.so")) return null;
        return mobileGlues;
    }

    public static void setupMobileGluesEnv(Context ctx, String renderer, Map<String, String> envMap) {
        if(!renderer.equals("mobileglues")) return;
        LibraryPlugin mobileGlues = getMobileGluesPlugin(ctx);
        if(mobileGlues == null) return;
        String mobileGluesLibrary = mobileGlues.resolveAbsolutePath("libmobileglues.so");
        envMap.put("LIBGL_ES", "3");
        envMap.put("POJAV_RENDERER", "opengles3");
        envMap.put("POJAVEXEC_EGL", mobileGluesLibrary);
        envMap.put("LIBGL_EGL", mobileGluesLibrary);
    ...
    public static void setEnviroimentForGame(Context context, String renderer) throws Throwable {
        ...
        setupMobileGluesEnv(context, renderer, envMap);
        // Init mesa renderers
        MesaUtils.initEnvironment(context, renderer, envMap);

        LibraryPlugin mobileGlues = renderer.equals("mobileglues") ? getMobileGluesPlugin(context) : null;
        String additionalLibraryPath = renderer.equals("mobileglues") && mobileGlues != null ?
                mobileGlues.getLibraryPath() : MesaUtils.getCustomZinkLibraryPath();
    ...
    public static String loadGraphicsLibrary(Context context, String renderer){
        ...
        switch (renderer){
            ...
            case "mobileglues":
                ...
                break;
            case "angle_vulkan":
                renderLibrary = findAnglePath(context, "vulkan");
                if (renderLibrary == null) return null;
                useGles = true;
            ...
            case "angle_directx":
                renderLibrary = findAnglePath(context, "directx");
                if (renderLibrary == null) return null;
                useGles = true;
                bypassNamespace = true;
                glesVersion = 3;
            ...
            case "swiftshader":
                renderLibrary = findSwiftShaderPath();
                if (renderLibrary == null) return null;
                useGles = true;
                glesVersion = 3;
                break;
            ...
            case "panfrost":
                renderLibrary = findPanfrostPath();
                if (renderLibrary == null) return null;
                useGles = true;
                glesVersion = 3;
                break;
            ...
            case "llvmpipe":
                renderLibrary = findLLVMpipePath();
                if (renderLibrary == null) return null;
                useGles = true;
                glesVersion = 3;
            break;
